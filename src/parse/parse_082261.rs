...
const GET_SOURCE: &str = "http://www.ibaraki-sirei.jp/saigai/naka/annai_list.html";
...
pub fn return_082261() -> Result<(), Box<dyn std::error::Error>> {
    println!("082261, 那珂市消防本部");
    let body = getsource()?;
    ...
    for element in document.select(&selector) {
        ...
        else if let Some((before, after)) = text.split_once("ころ那珂市") {
            // 時刻
    ...
    // JSONファイルに書き出し
    let mut file = File::create("dist/082261.json")?;
    file.write_all(output.to_string().as_bytes())?;
    eprintln!("{:?}", output);
    println!("JSONファイルが出力されました: 082261.json （那珂市消防本部）");
    Ok(())
}
