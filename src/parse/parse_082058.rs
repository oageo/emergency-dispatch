...
const GET_SOURCE: &str = "http://www.ibaraki-sirei.jp/saigai/ishioka/annai_list.html";
...
pub fn return_082058() -> Result<(), Box<dyn std::error::Error>> {
    println!("082058, 石岡市消防本部");
    let body = getsource()?;
    ...
    for element in document.select(&selector) {
        ...
        else if let Some((before, after)) = text.split_once("ころ石岡市") {
            // 時刻
    ...
    // JSONファイルに書き出し
    let mut file = File::create("dist/082058.json")?;
    file.write_all(output.to_string().as_bytes())?;
    eprintln!("{:?}", output);
    println!("JSONファイルが出力されました: 082058.json （石岡市消防本部）");
    Ok(())
}
