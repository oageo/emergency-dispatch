...
const GET_SOURCE: &str = "http://www.ibaraki-sirei.jp/saigai/omitama/annai_list.html";
...
pub fn return_082368() -> Result<(), Box<dyn std::error::Error>> {
    println!("082368, 小美玉市消防本部");
    let body = getsource()?;
    ...
    for element in document.select(&selector) {
        ...
        else if let Some((before, after)) = text.split_once("ころ小美玉市") {
            // 時刻
    ...
    // JSONファイルに書き出し
    let mut file = File::create("dist/082368.json")?;
    file.write_all(output.to_string().as_bytes())?;
    eprintln!("{:?}", output);
    println!("JSONファイルが出力されました: 082368.json （小美玉市消防本部）");
    Ok(())
}
