...
pub fn return_273660() -> Result<(), Box<dyn std::error::Error>> {
    println!("273660, 千早赤阪村（大阪南消防組合）");
    let body = getsource()?;
    ...
    for element in document.select(&selector) {
        ...
        if let Some((date_time, rest)) = text.split_once("ごろ、") {
            if let Some((location, disaster_info)) = rest.split_once("付近において、") {
                ...
                let address = if let Some(village_part) = location.split("村").nth(1) {
                    format!("大阪府千早赤阪村{}", village_part.trim())
                } else {
    ...
    let mut file = File::create("dist/273660.json")?;
    file.write_all(output.to_string().as_bytes())?;
    eprintln!("{:?}", output);
    println!("JSONファイルが出力されました: 273660.json （千早赤阪村、大阪南消防組合）");
    Ok(())
}
