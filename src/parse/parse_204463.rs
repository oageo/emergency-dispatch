...
pub fn return_204463() -> Result<(), Box<dyn std::error::Error>> {
    println!("204463, 麻績村（松本広域消防局）");
    ...
    let mut file = File::create("dist/204463.json")?;
    file.write_all(output.to_string().as_bytes())?;
    eprintln!("{:?}", output);
    println!("JSONファイルが出力されました: 204463.json （麻績村・松本広域消防局）");
    Ok(())
}
