...
const GET_SOURCE: &str = "http://www.ibaraki-sirei.jp/saigai/kitaibaraki/annai_list.html";
...
pub fn return_082155() -> Result<(), Box<dyn std::error::Error>> {
    println!("082155, 北茨城市消防本部");
    let body = getsource()?;
    ...
    for element in document.select(&selector) {
        ...
        else if let Some((before, after)) = text.split_once("ころ北茨城市") {
            // 時刻
    ...
    // JSONファイルに書き出し
    let mut file = File::create("dist/082155.json")?;
    file.write_all(output.to_string().as_bytes())?;
    eprintln!("{:?}", output);
    println!("JSONファイルが出力されました: 082155.json （北茨城市消防本部）");
    Ok(())
}
