...
const GET_SOURCE: &str = "http://www.kochi119.com/p/";

pub fn return_392014() -> Result<(), Box<dyn std::error::Error>> {
    println!("392014, 高知市消防局");
    ...
    if let Some(table_element) = document.select(&table_selector).next() {
        for td_element in table_element.select(&td_selector) {
            ...
            let address = lines[address_index]
                ...
                .unwrap_or("")
                .trim();
            let address = if address.starts_with("高知市") {
                format!("高知県{}", address)
            } else {
                address.to_string()
            };
    ...
    let mut file = File::create("dist/392014.json")?;
    file.write_all(output.to_string().as_bytes())?;
    eprintln!("{:?}", output);
    println!("JSONファイルが出力されました: 392014.json （高知市消防局）");
    Ok(())
}
