...
const GET_SOURCE: &str = "http://www.ibaraki-sirei.jp/saigai/daigo/annai_list.html";
...
pub fn return_083640() -> Result<(), Box<dyn std::error::Error>> {
    println!("083640, 大子町消防本部");
    let body = getsource()?;
    ...
    for element in document.select(&selector) {
        ...
        else if let Some((before, after)) = text.split_once("ころ大子町") {
            // 時刻
    ...
    // JSONファイルに書き出し
    let mut file = File::create("dist/083640.json")?;
    file.write_all(output.to_string().as_bytes())?;
    eprintln!("{:?}", output);
    println!("JSONファイルが出力されました: 083640.json （大子町消防本部）");
    Ok(())
}
