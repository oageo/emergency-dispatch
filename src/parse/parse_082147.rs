...
const GET_SOURCE: &str = "http://www.ibaraki-sirei.jp/saigai/takahagi/annai_list.html";
...
pub fn return_082147() -> Result<(), Box<dyn std::error::Error>> {
    println!("082147, 高萩市消防本部");
    let body = getsource()?;
    ...
    for element in document.select(&selector) {
        ...
        else if let Some((before, after)) = text.split_once("ころ高萩市") {
            // 時刻
    ...
    // JSONファイルに書き出し
    let mut file = File::create("dist/082147.json")?;
    file.write_all(output.to_string().as_bytes())?;
    eprintln!("{:?}", output);
    println!("JSONファイルが出力されました: 082147.json （高萩市消防本部）");
    Ok(())
}
