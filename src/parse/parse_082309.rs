...
const GET_SOURCE: &str = "http://www.ibaraki-sirei.jp/saigai/kasumigaura/annai_list.html";
...
pub fn return_082309() -> Result<(), Box<dyn std::error::Error>> {
    println!("082309, かすみがうら市消防本部");
    let body = getsource()?;
    ...
    for element in document.select(&selector) {
        ...
        if text.contains("発生はありません") {
        ...
        else if let Some((before, after)) = text.split_once("ころかすみがうら市") {
            // 時刻
            ...
            let location = if after.contains("付近") {
                ...
                if !parts.is_empty() {
                    format!("かすみがうら市{}付近", parts[0])
                } else {
                    format!("かすみがうら市{}", after)
                }
            } else {
                format!("かすみがうら市{}", after)
            };
    ...
    // JSONファイルに書き出し
    let mut file = File::create("dist/082309.json")?;
    file.write_all(output.to_string().as_bytes())?;
    eprintln!("{:?}", output);
    println!("JSONファイルが出力されました: 082309.json （かすみがうら市消防本部）");
    Ok(())
}
