...
const GET_SOURCE: &str = "http://www.ibaraki-sirei.jp/saigai/toride/annai_list.html";
...
pub fn return_082171() -> Result<(), Box<dyn std::error::Error>> {
    println!("082171, 常総地方広域市町村圏事務組合消防本部");
    let body = getsource()?;
    ...
    for element in document.select(&selector) {
        ...
        else if let Some((before, after)) = text.split_once("ころ取手市") {
            // 時刻
    ...
    // JSONファイルに書き出し
    let mut file = File::create("dist/082171.json")?;
    file.write_all(output.to_string().as_bytes())?;
    eprintln!("{:?}", output);
    println!("JSONファイルが出力されました: 082171.json （常総地方広域市町村圏事務組合消防本部）");
    Ok(())
}
