...
const GET_SOURCE: &str = "http://www.ibaraki-sirei.jp/saigai/hitachiomiya/annai_list.html";
...
pub fn return_082252() -> Result<(), Box<dyn std::error::Error>> {
    println!("082252, 常陸大宮市消防本部");
    let body = getsource()?;
    ...
    for element in document.select(&selector) {
        ...
        else if let Some((before, after)) = text.split_once("ころ常陸大宮市") {
            // 時刻
    ...
    // JSONファイルに書き出し
    let mut file = File::create("dist/082252.json")?;
    file.write_all(output.to_string().as_bytes())?;
    eprintln!("{:?}", output);
    println!("JSONファイルが出力されました: 082252.json （常陸大宮市消防本部）");
    Ok(())
}
